...
\addtocmark{Sturm–Liouville theory} % additional mark in the TOC
%
\mainmatter              % start of the contributions
%
\title{Sturm–Liouville theory}
%
\titlerunning{Sturm–Liouville theory}  % abbreviated title (for running head)
...
\begin{abstract}
The article is devoted to solution of Sturm–Liouville problem of operator dith distant perturbations in the space
\keywords{differential operators, spectrum, perturbations, eigenvalues}
\end{abstract}
%
\section{Introduction}
%
With this chapter, the preliminaries are over, and we begin the search
for periodic solutions to Sturm–Liouville proble. All this will be done in
the convex case; that is, we shall study the boundary-value problem
\begin{eqnarray*}
\left\{\Pi:=\{X:x_1\in \mathbb{R};x_2\in(0;\pi)\}\\
V=V(x),V\in C_0(\Pi)\\
W=W(x),W\in C_0(\Pi)\\
l\rightarrow+\infty\\
V(x_1-l,x2)+W(x_1+l, x_2) - perturbations\\


\left\{\begin{array}{l}(-\Delta+V(x_1-l, x_2)+W(x_1+l, x_2))\psi=\lambda\psi\\\psi(x_1, 0)=\psi(x_1, \pi)=0\end{array}\right.\\\\

We will prove solutions in form:\\

  (-\Delta+V(x)-\lambda_e)\Phi_+(x)+e^{-2l\sqrt{1-\lambda_e}}V(x)\Phi_-(+x_1+2l, x_2)e^{\sqrt{1-\lambda_e}\chi(x_1)}-\\
  \Phi_+(x)((1-\lambda_e)\chi'^2(x_1)-\sqrt{1-\lambda_e}\chi''(x_1))+2\frac{\partial}{\partial x_1}\Phi_+(x)\sqrt{1-\lambda_e}\chi'(x_1)&=&0\\
  \Phi_-(x)\sim\phi^-_1}+\phi^-_2+\phi^-_3+\dots\\
  \Phi_+(x)\sim\phi^+_0+\phi^+_1+\phi^+_2+\dots\\
  \lambda_e=1-\lambda_1-\lamba_2-\lambda_3-\dots\\
\end{eqnarray*}
